...
int main(int argc, char** argv) {
    ...
    #pragma omp target data map(to:W[0:(9)]) map(tofrom:N[0:(NX*NY*9)])
    ...
    for (int t = 0; t < 1000; t++) {
        ...
        #pragma omp target data map(to:N[0:(NX*NY*9)]) map(tofrom:workArray[0:(NX*NY*9)])
        #pragma omp target teams distribute parallel for collapse(3)
        for (int j = 0; j < NY; j++) {
            for (int i = 0; i < NX; i++) {
                for (int f = 0; f < 9; f++) {
        ...
        #pragma omp target data map(to:workArray[0:(NX*NY*9)]) map(tofrom:N[0:(NX*NY*9)])
        #pragma omp target teams distribute parallel for collapse(3)
        for (int j = 0; j < NY; j++) {
            for (int i = 0; i < NX; i++) {
                for (int f = 1; f < 9; f++) {
        ...
        #pragma omp target data map(to:SOLID[0:(NX*NY)], N[0:(NX*NY*9)]) map(tofrom:N_SOLID[0:(NX*NY*9)])
        #pragma omp target teams distribute parallel for collapse(2)
        for (int j = 0; j < NY; j++) {
            for (int i = 0; i < NX; i++) {
                if (SOLID[j * NX + i] == 1) {
        ...
        #pragma omp target data map(to:N[0:(NX*NY*9)]) map(tofrom:rho[0:(NX*NY)])
        #pragma omp target teams distribute parallel for collapse(2)
        for (int j = 0; j < NY; j++) {
            for (int i = 0; i < NX; i++) {
                rho[j * NX + i] = 0;
        ...
        #pragma omp target data map(to:N[0:(NX*NY*9)], rho[0:(NX*NY)]) map(tofrom:ux[0:(NX*NY)])
        #pragma omp target teams distribute parallel for collapse(2)
        for (int j = 0; j < NY; j++) {
            for (int i = 0; i < NX; i++) {
                ux[j * NX + i] = 0;
        ...
        #pragma omp target data map(to:N[0:(NX*NY*9)], rho[0:(NX*NY)]) map(tofrom:uy[0:(NX*NY)])
        #pragma omp target teams distribute parallel for collapse(2)
        for (int j = 0; j < NY; j++) {
            for (int i = 0; i < NX; i++) {
                uy[j * NX + i] = 0;
        ...
        #pragma omp target data map(to:ux[0:(NX*NY)], uy[0:(NX*NY)], cx[0:9], cy[0:9]) map(tofrom:workArray[0:(NX*NY*9)])
        #pragma omp target teams distribute parallel for collapse(3)
        for (int j = 0; j < NY; j++) {
            for (int i = 0; i < NX; i++) {
                for (int f = 0; f < 9; f++) {
        ...
        #pragma omp target data map(tofrom:workArray[0:(NX*NY*9)])
        #pragma omp target teams distribute parallel for collapse(3)
        for (int j = 0; j < NY; j++) {
            for (int i = 0; i < NX; i++) {
                for (int f = 0; f < 9; f++) {
        ...
        #pragma omp target data map(to:ux[0:(NX*NY)], uy[0:(NX*NY)]) map(tofrom:workArray[0:(NX*NY*9)])
        #pragma omp target teams distribute parallel for collapse(3)
        for (int j = 0; j < NY; j++) {
            for (int i = 0; i < NX; i++) {
                for (int f = 0; f < 9; f++) {
        ...
        #pragma omp target data map(to:workArray[0:(NX*NY*9)], W[0:(9)], rho[0:(NX*NY)]) map(tofrom:N[0:(NX*NY*9)])
        #pragma omp target teams distribute parallel for collapse(3)
        for (int j = 0; j < NY; j++) {
            for (int i = 0; i < NX; i++) {
                for (int f = 0; f < 9; f++) {
        ...
        #pragma omp target data map(to:N_SOLID[0:(NX*NY*9)]) map(tofrom:N[0:(NX*NY*9)])
        #pragma omp target teams distribute parallel for collapse(2)
        for (int j = 0; j < NY; j++) {
            for (int i = 0; i < NX; i++) {
                if (SOLID[j * NX + i] == 1) {

# source: https://github.com/naszy/PyPowerCube 
...
class cube_comm :
    ...
    def _decode(self, byte_str):    
        ...
        for b in byte_str:
            ...
            if b == '\x10' and len(byte_str)>11: # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                x = unpack("B", byte_str[i+1])[0]-0x80
                #print('x: '+str(x))
                #print(str(len(byte_str))+' '+str(i))
    ...
    def wait_until_pos_reached(self, xyz, pos, epsilon = 0.001):
        while True:
            #actual_position = plotter.get_pos_data(xyz)
            actual_position = self.get_pos_data(xyz)
            #print(str(pos)+' '+str(actual_position))
    ...
    def y_move_to(self, diff, vel, acc):
        ...
        if diff>=0 and diff<=1:
            ...
            print ("y must be between 0 and 1")    
    
    def z_move_to(self, diff, vel, acc):
    
        if diff>=0 and diff<=0.270:
            self._move_to('z', diff, vel, acc)
        else:
            print ("z must be between 0 and 0.270")
